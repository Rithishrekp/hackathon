...
const ProviderDashboard = () => {
    ...
    const stats = [
        { label: 'Total Earnings', value: '$0', icon: DollarSign, color: 'var(--success)' }, // Placeholder
        { label: 'Active Jobs', value: recentJobs.filter(j => j.status === 'upcoming' || j.status === 'pending').length.toString(), icon: Briefcase, color: 'var(--primary-600)' },
        { label: 'Completed', value: recentJobs.filter(j => j.status === 'completed').length.toString(), icon: CheckCircle, color: 'var(--accent-cyan)' },
        { label: 'Rating', value: '5.0', icon: Star, color: 'var(--accent-yellow)' },
    ];
            ...
            <div className="welcome-section">
                ...
                <div style={{ display: 'flex', gap: 'var(--space-4)', alignItems: 'center' }}>
                    ...
                    >
                        ...
                        {loading ? 'Updating...' : isAvailable ? 'Available Now' : 'Set as Available'}
                    </button>
